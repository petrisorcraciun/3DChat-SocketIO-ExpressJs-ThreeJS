...
THREE.MD2CharacterComplex = function() {
	...
	this.shareParts = (original) => {
		...
		// BODY
		let mesh = createPart(original.meshBody.geometry, this.skinsBody[0]);
		...
		for (let i = 0; i < original.weapons.length; i++) {
			let meshWeapon = createPart(original.weapons[i].geometry, this.skinsWeapon[i]);
	...
	this.loadParts = (config) => {
		...
		loader.load(config.baseUrl + config.body, (geo) => {
		...
		// WEAPONS
		let generateCallback = (index, name) => {
			return (geo) => {
	...
	this.setWireframe = (wireframeEnabled) => {
		if (wireframeEnabled) {
	...
	function loadTextures(baseUrl, textureUrls) {
		let mapping = new THREE.UVMapping();
		let textures = [];
	...
	function createPart(geometry, skinMap) {
		...
		let whiteMap = THREE.ImageUtils.generateDataTexture(1, 1, new THREE.Color( 0xffffff ));
		let materialWireframe = new THREE.MeshPhongMaterial({color: 0xffaa00, specular: 0x111111, shininess: 50, wireframe: true, shading: THREE.SmoothShading, map: whiteMap, morphTargets: true, morphNormals: true, metal: true });
		let materialTexture = new THREE.MeshPhongMaterial( {color: 0xffffff, specular: 0x111111, shininess: 50, wireframe: false, shading: THREE.SmoothShading, map: skinMap, morphTargets: true, morphNormals: true, metal: true });
		materialTexture.wrapAround = true;

		let mesh = new THREE.MorphBlendMesh(geometry, materialTexture);
